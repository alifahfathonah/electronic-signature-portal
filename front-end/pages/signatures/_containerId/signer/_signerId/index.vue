...
<script>
import VisualSigning from '@/components/company/VisualSigning'
export default {
  components: {
    VisualSigning
  },
  ...
  created () {
    this.$axios.$post(
      ...
      this.$nextTick(() => {
        this.$refs['pdf-signing'].loadPdf(this.files[0].content, response.signer.visual_coordinates)
      })
    })
  },
  methods: {
